...
const {generatePaymentID}= require("./utility-file");// extractSrcFromIframe
...
//const profileUpdateRouter = require("./profileUpdateRouter");
...
app.post("/users/register", async (req, res) => {
  //const { First_Name, Last_Name, Date_of_Birth, Email, Pass, User_Type, Bank_Account } = req.body;  
  ...
  //let User_ID;
    //await pool.query('CALL register_user($1, $2, $3, $4, $5, $6, $7,$8)', [First_Name, Last_Name, Date_of_Birth, Email, Pass, User_Type, Bank_Account,User_ID]);
...
app.get("/profile", async (req, res) => {
  try {
    ...
    let profileDetails;
    let courses;
    let fieldNames; // Define fieldNames here
    ...
    if (isStudent) {
      ...
      const { rows: fieldNamesResult } = await pool.query(fieldNamesQuery); // Rename the result variable to avoid conflict
      fieldNames = fieldNamesResult; // Assign fieldNamesResult to fieldNames
    }
...
//     console.log("the coure id is $1",courseId);
...
app.post("/submit-rating", isAuthenticated, async (req, res) => {
  try {
   ...
   // console.log("Existing rating result:", existingRatingResult.rows);

    if (existingRatingResult.rows.length > 0) {
      const ratingId = existingRatingResult.rows[0].rating_id;
    //  console.log("Existing rating ID:", ratingId);
     ...
     // console.log("Rating updated successfully");
      ...
      const insertRatingQuery = "INSERT INTO Course_Ratings (Rating_ID, User_ID, Course_ID, Rating) VALUES ($1, $2, $3, $4)";
      await pool.query(insertRatingQuery, [ratingId, userId, courseId, rating]);
     // console.log("New rating inserted successfully");
    }
...
app.post("/course/:courseId/chapter/:chapterId/addMaterial", async (req, res) => {
  try {
    ...
    //console.log("the retrieved link is $1",src);
...
app.get("/course/:courseId/chapter/:chapterId", async (req, res) => {
  try {
    const userId= req.session.userId;
    const userquery="select * users from user_id=$1";
    const users= await pool.query(userId);
    const user=users[0];
      const { courseId, chapterId } = req.params;
      ...
      const chapterQuery = "SELECT * FROM Course_Chapters WHERE Course_ID = $1 AND Chapter_ID = $2";
      const { rows: chapters } = await pool.query(chapterQuery, [courseId, chapterId]);
      ...
      res.render("chapter", { user, chapter, materials });
  } catch (error) {
      console.error("Error fetching chapter materials:", error);
      res.status(500).send("An error occurred while fetching chapter materials.");
  }
});
...
//app.use("/profile/update", require("./profileUpdateRouter"));
...
app.get('/topratedInstructors', async (req, res) => {
  try {
    ...
    const topRatedInstructors = topRatedInstructorsResult.rows; // Extract rows from the result
...
app.post("/course/:courseId/assignment/:assignmentId/submit", upload.single('submissionFile'), async (req, res) => {
  try {
    ...
    await pool.query(
      ...
      [assignmentId, studentId, submissionDate, true, null, submissionFile.filename] // Use filename or path, depending on your requirements
    );
...
// app.get("/course/:courseId/assignment/:assignmentId/evaluate", async (req, res) => {
//   try {
//     const submissions = await pool.query("SELECT * FROM Assignment_Submission WHERE assignment_id = $1", [req.params.assignmentId]);
//     res.render("evaluateSubmissions", { submissions });
//   } catch (error) {
//     console.error("Error fetching submissions for evaluation:", error);
//     res.status(500).send("An error occurred while fetching submissions for evaluation.");
//   }
// });
